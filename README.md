...
This is also enforced server-side: a `strip_markdown` function runs on every reply after the `SHOW_CARDS` line is stripped, as a backstop for the occasional turn where a model ignores the instruction regardless. Separately, `_FUNCTION_LEAK_RE` strips raw `<function=name>{...}` strings that Cerebras and Groq occasionally leak into response content when they should have emitted a structured `tool_calls` object — these are never executed and must never reach the customer as visible text.

### Malsha's service boundary

Culture & Services is the pavilion most prone to a specific failure mode: a customer asks for a *service* (visa help, a photographer, legal advice) and Kapruka's search returns books or accessories that share a word with the query. A book titled *"Sri Lanka Travel Guide"* is not a travel service. Malsha's prompt has a specific extra rule that covers this case: if a service-type search returns only physical products — especially books with tangentially related titles — treat it as a zero-match and write `SHOW_CARDS: none`. Tell the customer plainly that Kapruka doesn't stock that service type, then offer something genuinely relevant (e.g. a sworn translation service) or refer them elsewhere. This is distinct from the general zero-match rule because the result set isn't empty — it's full of plausible-looking but completely wrong things.

### Dev's anonymous sender option

The Lounge checkout has one addition the other shopkeepers don't: Dev specifically collects *who the order is from* — not just recipient details — and offers the customer the option to stay anonymous. The `sender` object on every order already supports an `anonymous: bool` flag; Dev is the only persona instructed to ask about it, because it's the one context where someone might actually want that. The cart-drawer checkout form collects a sender name as a required field for all pavilions, but the anonymous toggle is conversational-checkout-only and lives in Dev's `BEHAVIOUR RULES`.
...
- On first open, Sudu presents four preset navigation chips — *Need Groceries 🍎*, *Buying Clothes 👗*, *Looking for Medicine 💊*, *Buy a Gift Box 🎁* — so a visitor can get started with one tap rather than staring at a blank input. After any reply, `extractPavilionChips()` fires and the chips regenerate dynamically from her response, so subsequent turns always offer context-appropriate jumps rather than the same four defaults on loop.
...
A fifth route, `GET /`, serves as a lightweight health check: it returns `{status, service, gemini, cerebras, groq}` with boolean flags for which API keys are loaded, so the state of the backend is readable in one request without triggering any model or MCP calls. FastAPI's CORS middleware is configured with `allow_origins=["https://amirsohil.github.io"]` so the static GitHub Pages frontend can reach the Render backend without preflight issues.
...
- **20 results fetched, 8 cards shown.** Every `kapruka_search_products` call requests `limit: 20` rather than the API default. According to run tests, a specific query in Kapruka can bury its best matches behind popular generic bestsellers. Giving IDF reranking a 20-item pool before the model's `SHOW_CARDS` curation cuts it to 8 means the final visible cards are drawn from a meaningfully larger and better-sorted set without increasing noise for the customer.
- **`_query_matched: false` zero-match annotation.** The IDF step also runs a secondary check: after reranking, if none of the query's terms appear in any returned product name, it sets `"_query_matched": false` directly on the JSON payload before it reaches the model. This is distinct from the category-slug guard — the result set isn't empty, it's full of popular items that happen to match no part of the query (e.g. a search for "fish tank" returning cleaning supplies). Without the flag, the model receives a plausible-looking list and confidently recommends irrelevant products; with it, the `ZERO-MATCH RULE` in every persona's prompt tells the model to write `SHOW_CARDS: none` and acknowledge the gap honestly.
- **Today's date injected into every shopkeeper system prompt.** Without an explicit date anchor, Gemini, Cerebras, and Groq each infer "today" from their training data and can disagree with each other and with the actual calendar — which caused valid near-future delivery dates to be incorrectly rejected as "in the past" during testing. The fix: `date.today().strftime("%d %B %Y")` is appended to every non-Sudu system prompt at request time, giving the model a reliable, single source of truth for delivery-date validation. Sudu doesn't need it — she's tool-free and never handles orders.
...
- Cart context is echoed back on every request. `tool_history` resets to empty on every `/api/chat` call, which means a model answering turn N has no memory of what it found in turn N−1. For checkout this is fatal: the model needs a product ID from several turns back. The fix: the frontend sends a `cart` field on every request containing `{id, name, price, image}` for each item in the cart drawer. The backend injects these as a `CUSTOMER'S CONFIRMED CART` block in the system prompt, letting the shopkeeper call `kapruka_create_order` directly with confirmed IDs rather than re-searching — which could return a zero-match and falsely make the product look unavailable. Conversation history is trimmed to the last 10 turns per request; the cart injection is how checkout state survives that window.
- The cart-drawer form collects a sender name as a required field, separate from the recipient. This maps to the `sender.name` field in `kapruka_create_order`. Dev's conversational checkout additionally offers an anonymous sender option (handled through dialogue, not the form) — the one context in the market where someone might actually want it.
- The live delivery preview surfaces a `perishableWarning` when Kapruka flags temperature-sensitive constraints for the chosen city and date (e.g. same-day delivery required for chilled items). This appears inline in the preview before the order is placed.
- The city autocomplete dropdown shows known aliases alongside the canonical city name (e.g. *"Colombo (Col)"*), so the lookup works for how people actually type rather than requiring an exact official spelling.
- The Order Summary modal displays a payment link expiry time when Kapruka's response includes one: *"This payment link expires at HH:MM."* Kapruka's guest-checkout links are time-limited; surfacing the deadline before the customer navigates away prevents confusion if they try to use the link later.
